SET @add_user_session_id_sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE user_mitigation_measures ADD COLUMN user_session_id INT NULL AFTER id',
    'SELECT 1'
  )
...
SET @add_custom_hazard_id_sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE user_mitigation_measures ADD COLUMN custom_hazard_id INT NULL AFTER user_hazard_id',
    'SELECT 1'
  )
  FROM information_schema.columns
  WHERE table_schema = DATABASE()
...
SET @add_system_hazard_id_sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE user_mitigation_measures ADD COLUMN system_hazard_id INT NULL AFTER custom_hazard_id',
    'SELECT 1'
  )
  FROM information_schema.columns
  WHERE table_schema = DATABASE()
...
SET @add_additional_hazard_id_sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE user_mitigation_measures ADD COLUMN additional_hazard_id INT NULL AFTER system_hazard_id',
    'SELECT 1'
  )
  FROM information_schema.columns
  WHERE table_schema = DATABASE()
...
SET @relax_user_hazard_id_sql = (
  SELECT IF(
    is_nullable = 'NO',
    'ALTER TABLE user_mitigation_measures MODIFY user_hazard_id INT NULL',
    'SELECT 1'
  )
  FROM information_schema.columns
  WHERE table_schema = DATABASE()
