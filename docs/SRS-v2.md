...
| FR-CHZ-001 | The system shall validate platform co-created hazards across objective fit, mechanism fit, hazard definition, sector fit, country/region fit, and affected population groups. Mechanism fit requires a user-confirmed, source-supported causal linkage. | IMPLEMENTED | validation mode | [app/services/custom_hazard_validation.py :: validate_custom_hazard_dimensions()] |
...
| FR-CHZ-003 | The system shall require strict hazard validation to meet the configured ready score (default `75`) and dimension floor (default `7`). | IMPLEMENTED | `validation_mode=strict` | [app/config.py :: Settings.custom_hazard_validation_thresholds] |
| FR-CHZ-004 | The system shall allow easy hazard validation at the configured ready score (default `45`), dimension floor (default `3`), or because mode is easy after critical dimensions resolve. | IMPLEMENTED | `validation_mode=easy` | [app/config.py :: Settings.custom_hazard_validation_thresholds] [app/services/custom_hazard_validation.py :: _recommended_action()] |
...
Platform co-created hazard confidence scoring uses six weighted dimensions: hazard definition `0.25`, mechanism fit `0.20`, policy-objective fit `0.15`, selected sector fit `0.15`, country/region fit `0.10`, and affected population groups `0.15`. Policy-objective fit is mandatory and checks the hazard against the objective defined for the selected sector. [app/services/custom_hazard_validation.py :: DIMENSION_WEIGHTS] [app/services/custom_hazard_validation.py :: SECTOR_POLICY_OBJECTIVES]
...
The strict/easy toggle changes configurable ready scores and dimension floors for platform co-created hazards: strict mode defaults to a per-dimension floor of `7`, while easy mode defaults to `3`. The code read did not show a strict/easy branch that changes evidence contradiction invalidation. [app/config.py :: Settings.custom_hazard_validation_thresholds] [app/services/evidence_contradiction_service.py :: _normalize_verdict()]
...
| Strict ON requires every dimension supported, no contradictions; OFF allows insufficient if threshold met. | UNVERIFIED | Configurable strict/easy threshold logic exists for platform co-created hazards, but the complete claim across every hazard and mitigation dimension was not fully determined; contradiction handling is separate and not toggled by strict/easy in inspected evidence service. [app/config.py :: Settings.custom_hazard_validation_thresholds] [app/services/evidence_contradiction_service.py :: _normalize_verdict()] |
